...
const assignmentDetail = async (req, res) => {
  try {
    ...
    res.render('assignmentDetail', { schoolYear, lessons, assignment, updatedFiles, formattedStartDate, formattedDeadline, getSubmitDetail });
...
const showFileArray = async (req, res) => {
  try {
    ...
    // }
    if (contentType === "application/pdf") {
      res.render('viewFilesAdmin', { fileName });
    } else {
      res.render('viewFilesAdmin', { fileUrl: `/adminIndex/files/${assignId}/${fileIndex}`, contentType });
    }
